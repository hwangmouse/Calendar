...
import 'package:calendar/components/Assignment.dart';

class AssignmentAddScreen extends StatefulWidget {
  final Assignment? assignment;

  AssignmentAddScreen({this.assignment});
...
class _AssignmentAddScreenState extends State<AssignmentAddScreen> {
  ...
  void initState() {
    ...
    if (widget.assignment != null) {
      ...
      currentRatio = widget.assignment!.currentRatio.toString();
      isLateSubmissionAllowed = widget.assignment!.latePenalty > 0.0;
      latePenalty = widget.assignment!.latePenalty > 0.0
          ? widget.assignment!.latePenalty
          : null;
      isAlter = widget.assignment!.isAlter;
      deadline = widget.assignment!.deadline;
      expectedPeriod = widget.assignment!.expectedPeriod.toString();
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.assignment == null ? "과제 추가" : "과제 수정"),
      ),
      body: Padding(
          ...
          child: ListView(
            children: [
              ...
              ElevatedButton(
                ...
                child: Text(widget.assignment == null ? "과제 추가" : "과제 수정"),
              ),
            ],
          ),
        ),
